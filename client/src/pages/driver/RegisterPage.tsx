...
export default function RegisterPage() {
  ...
  if (step === 4) {
    return (
      <div className="bg-slate-100 min-h-screen flex items-center justify-center">
        <div className="w-full max-w-md min-h-screen sm:min-h-0 sm:h-212.5 bg-white flex flex-col items-center justify-center p-6 text-center sm:rounded-[2.5rem] sm:shadow-2xl overflow-hidden">
  ...
  return (
    <div className="bg-slate-100 min-h-screen flex items-center justify-center sm:py-8">
      <div className="w-full max-w-md min-h-screen sm:min-h-0 sm:h-212.5 bg-white flex flex-col sm:rounded-[2.5rem] sm:shadow-2xl overflow-hidden relative">
        <div className="w-full text-white px-6 pt-12 pb-8 rounded-b-4xl shadow-lg relative shrink-0" style={{ background: "oklch(0.22 0.07 255)" }}>
        ...
        <div className="px-6 py-6 flex-1 flex flex-col overflow-y-auto">
          ...
          {step === 2 && (
            <div className="space-y-4 flex-1">
              ...
              <div><label className="text-sm font-bold text-slate-800 mb-1.5 block">Plate Number</label><input type="text" value={plateNumber} onChange={(e) => setPlateNumber(e.target.value.toUpperCase())} placeholder="ABC 1234" className="w-full h-12 px-4 rounded-xl bg-slate-50 border border-slate-200 text-sm uppercase focus:bg-white focus:ring-2 focus:ring-navy-900 outline-none transition-all" /><p className="text-xs text-slate-400 mt-1">Must match LTO standard (e.g. ABC 123 or ABC 1234).</p></div>
              <div className="flex gap-3"><div className="w-1/2"><label className="text-sm font-bold text-slate-800 mb-1.5 block">Car Brand</label><select value={vehicleBrand} onChange={(e) => setVehicleBrand(e.target.value)} className="w-full h-12 px-4 rounded-xl bg-slate-50 border border-slate-200 text-sm focus:bg-white focus:ring-2 focus:ring-navy-900 outline-none appearance-none"><option value="" disabled>Select Brand</option>{ALLOWED_CAR_BRANDS.map(brand => <option key={brand} value={brand}>{brand}</option>)}</select></div><div className="w-1/2"><label className="text-sm font-bold text-slate-800 mb-1.5 block">Specific Model</label><input type="text" value={vehicleModel} onChange={(e) => setVehicleModel(e.target.value)} placeholder="e.g. Vios" className="w-full h-12 px-4 rounded-xl bg-slate-50 border border-slate-200 text-sm focus:bg-white focus:ring-2 focus:ring-navy-900 outline-none transition-all" disabled={!vehicleBrand} /></div></div>
              <div><label className="text-sm font-bold text-slate-800 mb-1.5 block">Vehicle Color</label><input type="text" value={vehicleColor} onChange={(e) => setVehicleColor(e.target.value)} placeholder="e.g. White" className="w-full h-12 px-4 rounded-xl bg-slate-50 border border-slate-200 text-sm focus:bg-white focus:ring-2 focus:ring-navy-900 outline-none transition-all" /></div>
              <div className="flex items-start gap-3 pt-2"><input type="checkbox" id="terms" checked={agreeTc} onChange={(e) => setAgreeTc(e.target.checked)} className="mt-0.5 w-4 h-4 shrink-0 rounded border-slate-300 focus:ring-2 focus:ring-navy-900 accent-navy-900" /><label htmlFor="terms" className="text-xs text-slate-600 leading-snug">I agree to the <span onClick={(e) => { e.preventDefault(); setShowTerms(true); }} className="font-bold cursor-pointer hover:underline" style={{ color: "oklch(0.22 0.07 255)" }}>Terms & Conditions</span> and <span onClick={(e) => { e.preventDefault(); setShowTerms(true); }} className="font-bold cursor-pointer hover:underline" style={{ color: "oklch(0.22 0.07 255)" }}>Privacy Policy</span>. I acknowledge that reservations are strictly <strong>non-refundable</strong> and only accept <strong>four-wheeled vehicles</strong>.</label></div>
