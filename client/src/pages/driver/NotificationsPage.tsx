...
export default function NotificationsPage() {
  ...
  // 1. Fetch notifications - Inayos para kumuha lang ng 'user' role at gamitin ang 'read' column
  const fetchNotifications = useCallback(async (userId: string) => {
    const { data } = await supabase.from("notifications")
      .select("*")
      .eq("user_id", userId)
      .eq("recipient_role", "user") // Filter para sa user lang
  ...
  // 3. Alert Sender - Dinagdag ang 'recipient_role' sa insert
  const sendUniqueNotif = async (userId: string, title: string, message: string, type: string, actionKey: string) => {
    ...
    if (!existing || existing.length === 0) {
      await supabase.from("notifications").insert({
        ...
        type,
        recipient_role: 'user', // Mahalaga para hindi makita ng admin
        read: false
      });
      toast.info(title);
    }
  ...
  // 5. Background Checker - Walang binago sa logic mo
  useEffect(() => {
    if (!user) return;
    ...
    >
      <div className="p-4 space-y-3 pb-24">
        ...
        {!loading && notifs.length === 0 && (
          <div className="text-center py-20 text-slate-400 text-[10px] font-black uppercase tracking-widest italic">
            All clear! No alerts.
          </div>
        )}
      </div>
