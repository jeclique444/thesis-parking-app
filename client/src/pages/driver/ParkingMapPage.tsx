...
import { MapPin, List, Map, Search, Loader2, Plus, Minus, Layers, Navigation, Route as RouteIcon, Crosshair, Star } from "lucide-react";
...
// Custom Icon
const createCustomIcon = (availableSlots: number, isClosed: boolean, isFavorite: boolean = false) => {
  if (isClosed) {
    return L.divIcon({
      html: `
  ...
  const starIcon = isFavorite ? '<div class="absolute -top-1 -right-1 bg-amber-400 rounded-full p-0.5"><svg width="10" height="10" viewBox="0 0 24 24" fill="white" stroke="none"><polygon points="12 2 15 9 22 9 16 14 18 21 12 17 6 21 8 14 2 9 9 9 12 2"/></svg></div>' : '';
                ...
                {filteredAndSorted.map((lot) => {
                  ...
                  return (
                    <div key={lot.id} className={cn("shrink-0 w-72 bg-white border border-slate-100 rounded-2xl p-3 shadow-sm flex flex-col gap-3", isClosed && "opacity-80")}>
                      <div onClick={() => !isClosed && navigate(`/parking/${lot.id}`)} className="flex flex-col gap-1">
                        <div className="flex items-center justify-between">
                          ...
                          <button onClick={(e) => toggleFavorite(lot.id, e)} className="p-1 hover:bg-gray-100 rounded-full">
                            <Star size={12} className={isFavorite ? "fill-amber-400 text-amber-400" : "text-gray-300"} />
                          </button>
                        </div>
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge variant="secondary" className="text-[9px] h-5 px-2 uppercase font-bold bg-gray-100">
                          {lot.type}
                        </Badge>
                        {isFavorite && (
                          <div className="bg-amber-100 px-1.5 py-0.5 rounded-full">
                    ...
                    {isClosed && (
                      <div className="text-right">
                        <p className="text-[10px] font-bold px-2 py-1 rounded-lg bg-gray-100 text-gray-500">CLOSED</p>
                      </div>
                    )}
                  </div>

                  {/* Stats Row: Slots, Distance, Time */}
                  ...
                  <div className="flex gap-2 mt-4">
                    <button 
                      ...
                      className="flex-1 bg-blue-500 hover:bg-blue-600 text-white py-2.5 rounded-xl text-[10px] font-bold flex items-center justify-center gap-1.5 transition-all"
                    >
                      <RouteIcon size={14} />
                      ROUTE
                    </button>
                    <button 
                      onClick={(e) => { e.stopPropagation(); openGoogleMaps(lot.latitude, lot.longitude, userCoords?.lat, userCoords?.lng); }}
                      className="flex-1 bg-green-500 hover:bg-green-600 text-white py-2.5 rounded-xl text-[10px] font-bold flex items-center justify-center gap-1.5 transition-all"
                    >
                      <Map size={14} />
                      GMAPS
                    </button>
                    <button 
                      onClick={(e) => { e.stopPropagation(); openWaze(lot.latitude, lot.longitude, userCoords?.lat, userCoords?.lng); }}
                      className="flex-1 bg-[#33CCFF] hover:bg-[#2DBBEA] text-white py-2.5 rounded-xl text-[10px] font-bold flex items-center justify-center gap-1.5 transition-all"
                    >
                      <Navigation size={14} />
                      WAZE
                    </button>
                    <button 
                      onClick={(e) => { e.stopPropagation(); toggleFavorite(lot.id, e); }}
                      className="w-10 bg-gray-100 hover:bg-gray-200 rounded-xl flex items-center justify-center transition-all"
                      title={isFavorite ? "Remove from favorites" : "Add to favorites"}
                    >
                      <Star size={16} className={isFavorite ? "fill-amber-400 text-amber-400" : "text-gray-400"} />
                    </button>
                  </div>
                </div>
              );
